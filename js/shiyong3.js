$.ajax({
	...
	success:function(data){
		console.log(data);
		var html="";
		for(var i=0;i<data.shiyong3.length;i++){
			html+=`
					<li class="adf">
						<img src="${data.shiyong3[i].img}" alt="" />
						<div class="${data.shiyong3[i].class}">
							<h4>${data.shiyong3[i].tittle}</h4>
							<p>
								<span>2032</span>
								<span>20台</span>
							</p>
							<p><span>1392</span>人申请</p>
							<p>${(data.shiyong3[i].tee.type===1)?("报告数量:"+data.shiyong3[i].tee.num):((data.shiyong3[i].tee.type===2)?("剩余时间"+data.shiyong3[i].tee.num+"天"):("查看试用名单"))}</p>
						</div>
									 <span>${data.shiyong3[i].span}</span>
   					</li>
			`;
			console.log(data.shiyong3.length);
		}
