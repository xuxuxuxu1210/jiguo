$.ajax({
	...
	success:function(data){
		console.log(data);
		var html="";
		for(var i=0;i<data.shiyong4.length;i++){
			html+=`
					<li class="adf">
						<img src="${data.shiyong4[i].img}" alt="" />
						<div class="${data.shiyong4[i].class}">
							<h4>${data.shiyong4[i].tittle}</h4>
							<p>
								<span>2032</span>
								<span>20台</span>
							</p>
							<p><span>1392</span>人申请</p>
							<p>${(data.shiyong4[i].tee.type===1)?("报告数量:"+data.shiyong4[i].tee.num):((data.shiyong4[i].tee.type===2)?("剩余时间"+data.shiyong4[i].tee.num+"天"):("查看试用名单"))}</p>
						</div>
									 <span>${data.shiyong4[i].span}</span>
   					</li>
			`;
		}
